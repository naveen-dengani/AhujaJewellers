...
import { Plus, X, Search, ChevronDown, Trash2, ArrowLeft } from "lucide-react";
import { formatCurrency, generateInvoiceNumber, calculateSimilarity } from "@/lib/utils";
...
export default function NewInvoicePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Pre-select customer if customerId param exists
      if (preselectedCustomerId) {
        const preselected = customersData.find((c: Customer) => c.id === preselectedCustomerId);
        if (preselected) {
  ...
  // Filtered products for new item
  ...
  return (
    <div>
      ...
      <style jsx>{`
        ...
        .selected-product { display: flex; justify-content: space-between; align-items: center; padding: 12px; background: var(--bg-secondary); border-radius: 8px; border: 1px solid var(--primary); }
